<template>
  ...
  <div class="card w-50">
    <div class="card-body">
      ...
      <small
        >Since the release of so called 'Passkeys' it is no longer needed to
        ...
        <a href="https://fidoalliance.org/passkeys/"
          >https://fidoalliance.org/passkeys/</a
        ></small
      >
    </div>
  </div>
...
export default {
  ...
  methods: {
    async register() {
      ...
      const user = await Parse.Cloud.run("registeruser", param);
      console.log(user);

      const token = await Parse.Cloud.run("askTokenForPasswordless", user);
      console.log(token);
